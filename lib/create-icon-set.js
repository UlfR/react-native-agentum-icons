...
const NativeIconAPI = (NativeModules.RNAgentumIconsManager || NativeModules.RNAgentumIconsModule);
  ...
  function getImageSource(name, size = DEFAULT_ICON_SIZE, color = DEFAULT_ICON_COLOR) {
    if (!NativeIconAPI) {
      if (Platform.OS === 'android') {
        throw new Error('RNAgentumIconsModule not available, did you properly integrate the module?');
      }
      throw new Error('RNAgentumIconsManager not available, did you add the library to your project and link with libRNAgentumIcons.a?');
    }
